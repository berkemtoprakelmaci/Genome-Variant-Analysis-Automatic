...
Must: Only one `.fasta` and only one `.fastq` or `.fastq.gz` file in same directory with bash; project-folder/ analysis_auto.sh  reference_genome.fasta  sample_data.fastq (or .fastq.gz)
